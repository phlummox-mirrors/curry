...
> intfEnts (HidingDataDecl _ _ _) = []
> intfEnts (IDataDecl _ _ _ cs xs) = filter (`notElem` xs) (concatMap ents cs)
>   where ents (ConstrDecl _ _ c _) = [c]
>         ents (ConOpDecl _ _ _ op _) = [op]
>         ents (RecordDecl _ _ c fs) = c : [l | FieldDecl _ ls _ <- fs, l <- ls]
> intfEnts (INewtypeDecl _ _ _ nc xs) = filter (`notElem` xs) (ents nc)
>   where ents (NewConstrDecl _ c _) = [c]
>         ents (NewRecordDecl _ c l _) = [c,l]
> intfEnts (ITypeDecl _ _ _ _) = []
